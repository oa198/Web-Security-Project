...
<div class="py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-semibold text-gray-900">Schedule Calendar</h1>
            <div class="flex space-x-2">
                <a href="{{ route('schedule.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 bg-white rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:text-gray-500 focus:outline-none focus:border-blue-300 focus:ring focus:ring-blue-200 active:text-gray-800 active:bg-gray-50 disabled:opacity-25 transition">
                ...
                <a href="{{ route('schedule.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-900 focus:outline-none focus:border-blue-900 focus:ring ring-blue-300 disabled:opacity-25 transition ease-in-out duration-150">
                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="flex justify-between items-center mb-6 bg-white p-4 rounded-lg shadow">
            <a href="{{ route('schedule.calendar', ['month' => $prevMonth, 'year' => $prevYear]) }}" class="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <a href="{{ route('schedule.calendar', ['month' => $nextMonth, 'year' => $nextYear]) }}" class="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                Next Month
        ...
        <div class="mb-6">
            <form action="{{ route('schedule.calendar') }}" method="GET" class="flex flex-wrap items-end space-x-0 md:space-x-4 space-y-4 md:space-y-0">
                ...
                <div class="flex items-end space-x-2">
                    ...
                    @if(request('term_id') || request('course_id'))
                    <a href="{{ route('schedule.calendar', ['month' => $month, 'year' => $year]) }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 active:bg-gray-400 focus:outline-none focus:border-gray-400 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150">
                        Clear
                    </a>
                    @endif
                </div>
        ...
        <div class="bg-white p-4 rounded-lg shadow mb-6">
            <h3 class="text-sm font-medium text-gray-700 mb-2">Schedule Types</h3>
            ...
            <div class="grid grid-cols-7 gap-px bg-gray-200">
                ...
                <div class="calendar-day {{ $day['current_month'] ? '' : 'inactive' }} p-2">
                    <div class="flex justify-between items-start">
                        ...
                        <span class="text-xs text-gray-500">{{ count($day['schedules']) }} class{{ count($day['schedules']) > 1 ? 'es' : '' }}</span>
                        @endif
                    </div>
                    <div class="mt-1">
                        @foreach($day['schedules'] as $schedule)
                        <div class="calendar-event event-{{ $schedule->type }}" title="{{ $schedule->course->code }} - {{ $schedule->start_time->format('g:i A') }} to {{ $schedule->end_time->format('g:i A') }}">
                            <a href="{{ route('schedule.show', $schedule->id) }}" class="text-gray-900 hover:text-blue-600">
                                {{ $schedule->course->code }} ({{ substr($schedule->type, 0, 1) }})
                            </a>
                        </div>
                        @endforeach
                    </div>
