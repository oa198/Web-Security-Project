...
<div class="py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="flex mb-5" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                        <a href="{{ route('schedule.show', $schedule->id) }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2">{{ $schedule->course->code }}</a>
                    </div>
                </li>
        ...
        <!-- Page Title -->
        <div class="mb-6">
            <h1 class="text-3xl font-semibold text-gray-900">Edit Schedule</h1>
            <p class="mt-2 text-sm text-gray-600">Update the schedule for {{ $schedule->course->code }} - {{ $schedule->course->name }}</p>
        </div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <form action="{{ route('schedule.update', $schedule->id) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-1">
                            ...
                            <select id="term_id" name="term_id" required class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                ...
                                @foreach($terms as $term)
                                    <option value="{{ $term->id }}" {{ old('term_id', $schedule->term_id) == $term->id ? 'selected' : '' }}>
                                        {{ $term->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-span-1">
                            ...
                            <select id="type" name="type" required class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                ...
                                <option value="tutorial" {{ old('type', $schedule->type) == 'tutorial' ? 'selected' : '' }}>Tutorial</option>
                                <option value="exam" {{ old('type', $schedule->type) == 'exam' ? 'selected' : '' }}>Examination</option>
                                <option value="other" {{ old('type', $schedule->type) == 'other' ? 'selected' : '' }}>Other</option>
                            </select>
                        </div>
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="text" name="room" id="room" value="{{ old('room', $schedule->room) }}" required class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        </div>
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="time" name="start_time" id="start_time" value="{{ old('start_time', $schedule->start_time ? $schedule->start_time->format('H:i') : '') }}" required class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        </div>

                        <!-- End Time -->
                        <div class="col-span-1">
                            <label for="end_time" class="block text-sm font-medium text-gray-700">End Time <span class="text-red-500">*</span></label>
                            <input type="time" name="end_time" id="end_time" value="{{ old('end_time', $schedule->end_time ? $schedule->end_time->format('H:i') : '') }}" required class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        </div>

                        <!-- Schedule for Specific Date (for exams or one-time events) -->
                        <div class="col-span-1 {{ $schedule->type != 'exam' && $schedule->is_recurring ? 'hidden' : '' }}">
                            <label for="specific_date" class="block text-sm font-medium text-gray-700">Specific Date</label>
                            <input type="date" name="specific_date" id="specific_date" value="{{ old('specific_date', $schedule->specific_date ? $schedule->specific_date->format('Y-m-d') : '') }}" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            <p class="mt-1 text-xs text-gray-500">For one-time events only (leave blank for recurring weekly schedule)</p>
                        </div>

                        <!-- Recurring Schedule -->
                        <div class="col-span-1 {{ $schedule->type == 'exam' ? 'hidden' : '' }}">
                            <div class="flex items-start mt-7">
                                <div class="flex items-center h-5">
                                    <input id="is_recurring" name="is_recurring" type="checkbox" value="1" {{ old('is_recurring', $schedule->is_recurring) ? 'checked' : '' }} class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded">
                                </div>
                        ...
                        <div class="col-span-2">
                            ...
                            <textarea id="notes" name="notes" rows="3" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('notes', $schedule->notes) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="px-4 py-3 bg-gray-50 text-right sm:px-6 flex justify-end space-x-2">
                    <a href="{{ route('schedule.show', $schedule->id) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        Cancel
                    </a>
