...
<div class="py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-semibold text-gray-900">Courses</h1>
        ...
        <div class="mb-6">
            <form action="{{ route('courses.index') }}" method="GET" class="flex items-center">
                <div class="relative flex-grow">
                    ...
                    @if(request('search'))
                    <a href="{{ route('courses.index') }}" class="absolute right-0 top-0 mt-2 mr-2 text-gray-600 hover:text-gray-800">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-lg hover:shadow-md transition-shadow duration-300">
                <div class="p-6">
                    <div class="flex justify-between items-start">
                        ...
                        <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $course['enrolled'] ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                            {{ $course['enrolled'] ? 'Enrolled' : 'Not Enrolled' }}
                        </span>
                    </div>
                    
                    <div class="mt-4">
                        <p class="text-sm text-gray-700"><span class="font-medium">Instructor:</span> {{ $course['instructor'] ?: 'TBA' }}</p>
                        <p class="text-sm text-gray-700"><span class="font-medium">Credits:</span> {{ $course['credits'] }}</p>
                    </div>
                    
                    <div class="mt-4 text-sm text-gray-700">
                        <p class="font-medium">Schedule:</p>
                        <p>{{ implode(', ', $course['schedule']['days']) }} | {{ $course['schedule']['startTime'] }} - {{ $course['schedule']['endTime'] }}</p>
            ...
            <div class="col-span-3 text-center py-12">
                ...
                <p class="mt-1 text-sm text-gray-500">Try adjusting your search criteria or create a new course.</p>
                @can('create', App\Models\Course::class)
                <div class="mt-6">
